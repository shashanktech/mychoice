...
$sql_college_type= "SELECT * FROM college_types";
$result_college_type= mysql_query($sql_college_type) OR DIE(mysql_error());

$sql_college_course = "SELECT * FROM college_course_selection";
$result_college_course = mysql_query($sql_college_course) or die(mysql_error());

if (isset($_POST['Add'])) {
$college_course_name= $_POST['college_course_name'];
$college_type = $_POST['college_type'];
$sql_college_type= "INSERT INTO `college_course_selection`(`course_name`,`course_category`) VALUES('".$college_course_name."','".$college_type."')";
$result_college_type= mysql_query($sql_college_type) OR DIE(mysql_error());
header("location:add_college_courses.php"); 
}
if(isset($_POST['Delete'])){
  if(!empty($_POST['checkbox'])){
    foreach ($_POST['checkbox'] as $selected) {
      $sql_delete= "Delete FROM college_course_selection where course_name='".$selected."'".";";
              ...
              <form role="form" id="templatemo-preferences-form" method="POST">
                <div class="row">
                  <div class="col-md-6 margin-bottom-15">
                    <label for="firstName" class="control-label">Add New College Course</label>
                    <input type="text" class="form-control" name="college_course_name" id="firstName">
                    </div>                 
                 </div>
                  <div class="row">
                  <div class="col-md-6 margin-bottom-15">
                  <label for="singleSelect">Select Preferred College Type For Course</label>
                      <?php
                      $sql_college_type = "SELECT college_type FROM college_types";
                      $result_college_type  = mysql_query($sql_college_type) OR DIE(mysql_error());
                      ?>
                      <select class="form-control margin-bottom-15" id="singleSelect" name="college_type">
                ...
                <div class="col-md-6 ">
                <label> Added College Courses </label>
                <?php while($row_college_course = mysql_fetch_array($result_college_course)){ ?>
                  <div class="checkbox">
                    <label>
                      <input type="checkbox" name="checkbox[]" value="<?php echo $row_college_course['course_name'] ?>">
                     <?php echo  $row_college_course['course_name'] ?> FOR <?php echo $row_college_course['course_category'] ?>
                    </label>

...
if (isset($_POST['Add'])) {
$college_type_name= $_POST['college_type_name'];
$sql_college_type= "INSERT INTO `college_types`(`college_type`) VALUES('".$college_type_name."')";
$result_college_type= mysql_query($sql_college_type) OR DIE(mysql_error());
header("location:add_college_type.php"); 
}
if(isset($_POST['Delete'])){
  if(!empty($_POST['checkbox'])){
    foreach ($_POST['checkbox'] as $selected) {
      $sql_delete= "Delete FROM college_types where college_type='".$selected."'".";";
              ...
              <form role="form" id="templatemo-preferences-form" method="POST">
                <div class="row">
                  <div class="col-md-6 margin-bottom-15">
                    <label for="firstName" class="control-label">Add New College Type</label>
                    <input type="text" class="form-control" name="college_type_name" id="firstName">
                    <button type="submit" id="Add" name="Add" class="btn btn-primary">Add</button>
                    </div>                 
                 </div>
                <div class="col-md-6 ">
                <label> Added College Types </label>
                <?php while($row_college_type = mysql_fetch_array($result_college_type)){ ?>
                  <div class="checkbox">
                    <label>
                      <input type="checkbox" name="checkbox[]" value="<?php echo $row_college_type['college_type'] ?>">
                     <?php echo  $row_college_type['college_type'] ?>
                    </label>
